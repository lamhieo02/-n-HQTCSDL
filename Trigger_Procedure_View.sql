...
alter view StudentScheduleView 
as

select cs.Username as 'Mã học viên', cl.ID as 'Mã lớp', cl.Name as 'Tên lớp',c.Name as 'Tên khóa học' ,t.Name as 'Tên giáo viên', cl.WeekDays as 'Ngày học', CONCAT(SUBSTRING(convert(varchar, cl.Start_Time ,108),1,5),' : ' ,SUBSTRING(convert(varchar, cl.End_Time ,108),1,5)) as 'Thời gian học', cl.ClassRoom as 'Phòng học'
...
alter procedure getScheduleStudent (@name varchar(100))
as begin 
select * from StudentScheduleView where StudentScheduleView.[Mã học viên] = @name
end
...
select t.Username as 'Username', cl.ID as 'Mã lớp', cl.Name as 'Tên lớp',cl.Course_ID as 'Mã khóa học',c.Name as 'Tên khóa học' , cl.WeekDays as 'Ngày học', CONCAT(SUBSTRING(convert(varchar, cl.Start_Time ,108),1,5),' : ' ,SUBSTRING(convert(varchar, cl.End_Time ,108),1,5)) as 'Thời gian học', cl.ClassRoom as 'Phòng học'
